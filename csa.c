...
static void csa_ComputeKey( uint8_t *pkk/*kk[57]*/, uint8_t ck[8] );
 ...
 * csa_SetCW:
 *****************************************************************************/
#if 0 /* providing the key by a string is not needed here */
...
#endif
int csa_SetCW(vlc_object_t *p_caller, csa_t *c, char *pck, bool set_odd )
{
   if( set_odd )
   {
      memcpy( c->o_ck , pck, 8 );
      csa_ComputeKey( &(c->o_kk), pck );
   }
   else
   {
      memcpy( c->e_ck , pck, 8 );
      csa_ComputeKey( &(c->e_kk) , pck );
   }
